...
def inicio_empleador(master, usuario= None): 
    ...
    contenedor_notificaciones = ctk.CTkFrame(ventana, fg_color="#FFFFFF")
    ...
    def mostrar_notificaciones_empleador(parent, db, usuario):
        try:
            notificaciones = Notificaciones.historial_nots(db, usuario['ID'], leida=False)
            
            if not notificaciones:
                ctk.CTkLabel(parent,
                    text="No tienes notificaciones pendientes",
                    font=("Open Sans", 14)).pack(pady=50)
                return
...
def inicio_trabajador(master, usuario=None):
    ...
    def mostrar_nueva_contratacion(parent, notif, db):
            """Muestra los detalles de una nueva contratación"""
            try:
                ...
                if not contrataciones:
                    raise ValueError("Contratación no encontrada")
                
                contratacion = next(
                    (c for c in contrataciones if c['ID_contratacion'] == notif.get('ID_contratacion')), None)
                ...
                detalles = [
                    ...
                    f"Empleador: {contratacion['empleador'].get('nombre', 'Desconocido')}",
                    f"Modalidad: {contratacion.get('modalidad', 'No especificada')}",
                    f"Valor acordado: ${contratacion.get('valor_acordado', 0):,.2f}"
                ]
                
                for detalle in detalles:
                    ctk.CTkLabel(detalles_frame, text=detalle, font=("Open Sans", 12)).pack(anchor="w")
                ...
                def aceptar():
                    ...
                    Notificaciones.marcar_leido(db, notif['ID_notificacion'])
                    messagebox.showinfo("Aceptado", "Has aceptado la contratación")
                    parent.destroy()

                def rechazar():
                    
                    Notificaciones.marcar_leido(db, notif['ID_notificacion'])
                    messagebox.showinfo("Rechazado", "Has rechazado la contratación")
                    parent.destroy()

                ctk.CTkButton(btn_frame, text="Aceptar", fg_color="green", command=aceptar).pack(side="left", expand=True, padx=5)
                ctk.CTkButton(btn_frame, text="Rechazar", fg_color="red", command=rechazar).pack(side="right", expand=True, padx=5)

            except Exception as e:
                messagebox.showerror("Error", f"No se pudo mostrar la contratación: {str(e)}")
    ...
    def mostrar_notificacion_individual(parent, notif, db, usuario):
        ...
        if notif['tipo'] == 'oferta_pendiente':
            ...
            mostrar_nueva_contratacion(frame, notif, db)
    ...
    def mostrar_notificaciones_trabajador(parent, db, usuario):
        ...
        try:
            ...
            contenedor_scroll = ctk.CTkFrame(parent)
            contenedor_scroll.grid(row=0, column=0, sticky="nsew")  # En lugar de pack()
            ...
            for notif in notificaciones:
                mostrar_notificacion_individual(scrollable_frame, notif, db, usuario)
    ...
    def mostrar_oferta_pendiente(parent, notif, db, usuario):
        ...
        try:
            ...
            ctk.CTkButton(btn_frame,
                ...
                command=lambda: responder_oferta(True)).pack(side="left", padx=5, fill="x", expand=True)
            
            ctk.CTkButton(btn_frame,
                text="Rechazar", fg_color="#F44336",
                command=lambda: responder_oferta(False)).pack(side="right", padx=5, fill="x", expand=True)
        ...
        def procesar_respuesta_oferta(parent_window, id_notificacion, aceptada, db, usuario):
            try:
                ...
                Notificaciones.marcar_leido(db, id_notificacion)
