...
class Database:
    ...
    def obtener_notificaciones_empleador(self, id_empleador):
        notificaciones = []
        for noti in self.data.get("notificaciones", []):
            if noti.get("ID_empleador") == id_empleador and noti.get("tipo_nots") in ["contratacion_aceptada", "contratacion_rechazada"]:
...
class Contratacion:
    ...
    def actualizar_estado(db, ID_contratacion, nuevo_estado):
        ...
        actualizada = False
        for c in db.data['contrataciones']:
    ...
    def cambiar_estado(self, db: Database, nuevo_estado: str, motivo: str = "") -> bool:
        estado_actual = self.estado_contratacion.lower()
        
        if nuevo_estado.lower() not in self.ESTADOS_VALIDOS.get(estado_actual, []):
            return False
            
        for i, contratacion in enumerate(db.data['contrataciones']):
            if contratacion['ID_contratacion'] == self.ID_contratacion:
                db.data['contrataciones'][i]['estado_contratacion'] = nuevo_estado
                
                mensaje = f"Contratación {nuevo_estado}"
                if motivo:
                    mensaje += f". Motivo: {motivo}"
                    
                if nuevo_estado == 'aceptada':
                    destinatario = self.ID_empleador
                    tipo_not = 'contratacion_aceptada'
                elif nuevo_estado == 'rechazada':
                    destinatario = self.ID_empleador
                    tipo_not = 'contratacion_rechazada'
                else:
                    destinatario = self.ID_trabajador if estado_actual == 'pendiente' else self.ID_empleador
                    tipo_not = 'estado_contratacion'
                
                Notificaciones.crear_notificacion_contratacion(
                    db, self.ID_contratacion, mensaje, tipo_not, destinatario)
                
                return db.save_data()
        return False

    @staticmethod
    def obtener_contrataciones_completas(db: Database, ID_usuario: str = None, 
                                       tipo_usuario: str = None, estado: str = None) -> List[Dict]:
        resultados = []
        for contratacion in db.data['contrataciones']:
            match = True
            if ID_usuario and tipo_usuario == 'empleador' and contratacion['ID_empleador'] != ID_usuario:
            ...
            if match:
                ...
                empleador = db.get_empleador_by_id(contratacion['ID_empleador'])
                usuario_empleador = db.get_user_by_id(contratacion['ID_empleador'])
                trabajador = db.get_trabajador_by_id(contratacion['ID_trabajador'])
                usuario_trabajador = db.get_user_by_id(contratacion['ID_trabajador'])
                servicio = None
                if contratacion.get('ID_servicio'):
                    servicio = next((s for s in db.data['servicios'] 
                                   if s['ID_servicio'] == contratacion['ID_servicio']), None)
                
               
                notificaciones = Notificaciones.obtener_notificaciones_usuario(
                    db, ID_usuario, filtro_tipo=None, no_leidas=False)
                notifs_relacionadas = [n for n in notificaciones 
                                      if n.get('ID_contratacion') == contratacion['ID_contratacion']]
                ...
                contratacion_data = {
                    **contratacion,
                    ...
                    'servicio': servicio,
                    'notificaciones': notifs_relacionadas
                }
                resultados.append(contratacion_data)
        return resultados
...
class Notificaciones:
    def __init__(self, ID_destinatario=None, ID_emisor=None, mensaje=None, 
                 tipo=None, ID_contratacion=None, ID_notificacion=None, 
                 fecha=None, estado="pendiente", datos_adicionales=None):
    ...
    @staticmethod
    def enviar_notificacion(db, id_emisor, id_receptor, tipo, mensaje, id_contratacion=None):
        """Envía notificación (método estático alternativo)"""
        ...
        return notif.enviar_nots(db)

    @staticmethod
    def marcar_como_leida(db, id_notificacion):
        try:
            ...
            with open(csv_path, mode='r', encoding='utf-8') as f:
                reader = csv.DictReader(f)
                for row in reader:
                    if row['ID_notificacion'] == id_notificacion:
                        row['leida'] = 'True'  
                        updated = True
            ...
            if updated:
                with open(csv_path, mode='w', encoding='utf-8', newline='') as f:
                    writer = csv.DictWriter(f, fieldnames=notificaciones[0].keys())
                    writer.writeheader()
                    writer.writerows(notificaciones)
                return True
    ...
    @staticmethod
    def obtener_notificaciones_usuario(db: Database, ID_usuario: str, filtro_tipo: str = None, 
                                    no_leidas: bool = False) -> List[Dict]:
        resultados = []
        for notif in db.data.get('notificaciones', []):
            if notif.get('ID_destinatario') != ID_usuario:
                continue
            if filtro_tipo and notif.get('tipo') != filtro_tipo:
                continue
            if no_leidas and notif.get('estado') == "leido":
                continue
            resultados.append(notif)
        return sorted(resultados, key=lambda x: x.get('fecha', ''), reverse=True)

    @staticmethod
    def crear_notificacion_contratacion(db: Database, ID_contratacion: str, mensaje: str, 
                                      tipo: str, ID_destinatario: str) -> bool:
        contratacion = next((c for c in db.data['contrataciones'] 
                           if c['ID_contratacion'] == ID_contratacion), None)
        if not contratacion:
            return False
        ...
        nueva_notificacion = Notificaciones(
            ...
            ID_emisor=contratacion['ID_empleador'] if tipo == "nueva_contratacion" else contratacion['ID_trabajador'],
            mensaje=mensaje,
            tipo=tipo,
        ...
        return nueva_notificacion.enviar_nots(db)
